...
while True:
    ...
    command_output = api.create_chat_completion(
        "gpt-3.5-turbo", messages, temperature=0
    )

    # Show the command returned by the model to be executed on the command line interface
    click.echo(click.style("Output: ", fg="yellow") + command_output)
    click.echo()
    ...
    if choice == "y":
        ...
        if r != 0:
            click.echo(click.style("Error executing command.", fg="red"))
    else:
        continue
